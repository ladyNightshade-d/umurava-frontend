...
const CandidateAuth = () => {
  ...
  // M-10: onGoogleLogin properly inside component — no dead module-level function
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // M-2: exponential back-off lock
      const newFail = failCount + 1;
      setFailCount(newFail);
      if (newFail >= 5) {
        const lockMs = Math.min(30000 * Math.pow(2, newFail - 5), 300000);
      ...
      <Card className="w-full max-w-md">
        ...
        <CardContent>
          ...
          <div className="mt-3 text-center">
            <Link href="/auth" className="text-xs text-muted-foreground hover:underline" rel="noopener noreferrer">
              Are you a recruiter? →
            </Link>
          </div>
        </CardContent>
